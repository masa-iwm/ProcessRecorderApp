...
- `Does the bitrate reach the GPU encoder?` 節 ── 同じソースを `bitrate=500` と `bitrate=4000` で録った 2 本の**実測ビットレート（MP4 バイト数 ÷ 尺）と、その比**。コンテナのオーバーヘッドを含むので絶対値は要求値より上に出る。**読むのは比だけで、期待は高/低 ≥ 2**。**判定は自動化していない**（320x240 のソースは高い目標を必要としないことがあり、尺の欄自体もノイズが多い。ここを赤くすると「このエンコーダーは壊れている」という誤った記録が残る）ので、終了コードには反映されない ── 目で読むこと。
- **比が 2 を大きく下回ったときに疑うのは単位ではなくレート制御**である。`bitrate` を拘束力のあるものにしているのは `rate-control=cbr`（qsv）/ `rc-mode=cbr`（nvcodec）で、代替は `vbr` ＋ `max-bitrate`。
- **決定点（この実行で必ず見るもの）** ── 既定録画は CBR 2000 kbit/sec で走り、**この値は解像度にも fps にも依らない固定値**である。次の 3 つを見て、カタログを `vbr` ＋ `max-bitrate` へ倒すかをここで決める:
  - `automatic selection` のケースの絵（既定録画の目視品質）。
  - **4K 録画の画質** ── `Verify-HighResolution.ps1` の 4K ケースがまさに CBR 2000 kbit/sec の構成で回る。2000 kbit/sec は 4K には低いので、破綻していないかを目で見る。
  - **常時録画セグメントのサイズ** ── 常時枝も同じ起動文字列（GOP だけ枝のレート基準）なので、低 fps・低解像度の枝でも目標 2000 kbit/sec まで膨らみうる。`Verify-HighResolution.ps1` の常時録画ケースが出す MP4 のバイト数と尺を見て、ディスク使用量が許容範囲かを確かめる。
