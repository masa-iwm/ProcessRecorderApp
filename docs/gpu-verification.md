...
- ケースごとに `settings.json` を生成 → 常駐ワーカー起動 → 録画 → 停止 → MP4 を ISO-BMFF の直接パースで検証（外部プロセス不要）。合格条件は `ftyp`・`moov`・`mdat` の各ボックスが在ること、`moov` 内に `avcC`（H.264 のデコーダー構成）が含まれること、再生時間が 0 より大きいこと。
...
| `D3d12 / <GPU エンコーダー> bitrate=500 max-bitrate=750` と `bitrate=4000 max-bitrate=6000` | **ビットレートが本当にエンコーダーへ届くかを測る 2 本**（peak は目標の 1.5 倍 ── 製品が当てる比と同じ）。`bitrate` の単位が確認できている GPU エンコーダー（`qsvh264enc` / `nvh264enc` / `nvd3d11h264enc` / `nvautogpuh264enc`）が実機に在れば、それぞれ低・高の 2 ケースが自動生成される。起動文字列は `$bitrateTemplates`（カタログと L1 で一致を固定）から完全な形で組み、`EncodingProperties` として渡す ── `parse_launch` へ生補間されるので、カタログの `bitrate=` の上に 2 つ目の代入を重ねる形（last-wins 頼み）にはしない。`d3d12h264enc` / `amfh264enc` は `bitrate` を持たないのでケースが作られない |
...
- エンコーダーの一覧はカタログと L1 の `EncoderCatalogScriptSyncTests` で固定されている: `$allEncoders`（問い合わせる全名）・`$gpuEncoders`（GPU 専用候補）・`$manualCandidates`（名前・プロパティ文字列・順序）・`$bitrateTemplates`（対象の名前と、`{0}` に既定の 2000 を入れた文字列）・要素列挙の正規表現、さらに `Verify-HighResolution.ps1` のエンコーダー行まで。カタログの候補を足す・変えるとこれらのテストが落ちてスクリプト側の追随を強制する ── 「カタログを直してスクリプトの一覧を直し忘れる」事故の再発防止なので、テストを黙らせるのではなく一覧を直すこと。
...
- `duration` ── 尺は `moov` の `mvhd` から採り、それが 0 のとき（fragmented MP4 ── `FragmentedOutput` の既定）は sidecar `<ファイル>.mp4.json` の `durationMs` を使う。`(sidecar)` と付いた欄が fragmented の録画物で、付かない欄は fragmented でない。**どちらからも尺が取れなければそのケースは失敗**（現行の製品はどの録画にも sidecar を書くため、無いこと自体が異常）。ビットレートの実測（MP4 バイト数 ÷ 尺）と `Short recordings` 節もこの尺を使う。
...
- `Does the bitrate reach the GPU encoder?` 節 ── 同じソースを目標/ピーク `500/750` と `4000/6000` kbit/sec で録った 2 本の**実測ビットレート（MP4 バイト数 ÷ 尺）と、その比**。コンテナのオーバーヘッドを含むので絶対値は要求値より上に出る。**これは VBR なので「高/低 ≥ 2」では読まない** ── VBR は静止した 320x240 のソースで目標まで出す義理が無く、低い側には下限も無い。読み方は **高 > 低** と、**高がその peak（6000 kbit/sec）を大きく超えていないこと**の 2 つ。コンテナのオーバーヘッドを含むので peak をわずかに上回るのは正常で、桁で外れていたら値が意図したプロパティに入っていない（**レート制御より先に単位を疑う**）。**判定は自動化していない**（尺の欄がノイズを含み、ここを赤くすると「このエンコーダーは壊れている」という誤った記録が残る）ので、終了コードには反映されない ── 目で読むこと。
- **高と低が同じ値に出たときに疑うのはレート制御**である。`bitrate` と `max-bitrate` を拘束力のあるものにしているのは `rate-control=vbr`（qsv）/ `rc-mode=vbr`（nvcodec）/ `rc-mode=pcvbr`（mediafoundation）で、`cqp` / `icq` は `bitrate` を丸ごと無視する。
- **決定点（この実行で必ず見るもの）** ── 既定録画は**サイズ連動 VBR** で走る（`幅 × 高さ × fps × 0.1 / 1000` kbit/sec、床 300・天井 40000、ピークは目標の 1.5 倍）。GPU 実機でこの形が走るのはこの実行が初めてなので、次の 4 つを見て係数（`EncoderCatalog.BitsPerPixel`）とピーク比（`VbrPeakRatio`）を据え置くかをここで決める:
  - `automatic selection` のケースの絵（既定録画の目視品質）。`gst.encoder selected` の `bitrate-kbps=` と `size-source=` を併せて読む ── `size-source=assumed` が出ていたら、その機械では大きさを読めておらず、絵の良し悪しは係数の話ではない。
  - **4K 録画（約 25 Mbps）の画質とファイルサイズ** ── 3840x2160@30 の式の値は 24883 kbit/sec、ピーク 37325。`Verify-HighResolution.ps1` の 4K スイープ行は 15fps ぶんの式の値（12442 / 18663）を明示して回る。**画質が過剰なら係数を下げ、ディスクが問題なら天井を下げる** ── 判断が別なので、どちらの理由で不満なのかを分けて記録する。
  - **常時録画セグメントのサイズ** ── 常時枝は**枝の大きさと枝のレート**で式を解くので、960x540@5fps は式では 259 kbit/sec ── 床の 300 に当たる。`Verify-HighResolution.ps1` の常時録画ケースが出す MP4 のバイト数と尺を見て、ディスク使用量と画質の釣り合いを確かめる（**低すぎるなら床 `MinBitrateKbps` を上げる**）。
  - **`max-bitrate` が実際に効いているか** ── 上の比の節。高い側が自分の peak を桁で超えていたら単位を疑う。
- 採用された `LaunchString` を見て、`EncoderCatalog` の実機未確認エンコーダー（`d3d12h264enc` / `amfh264enc`。現在 GOP 長のみ）に確認済みのプロパティを追加できるか判断する。単位（kbit/sec か bit/sec か）は `gst-inspect-1.0.exe <要素名>` で必ず確認する。
...
ケースは20件（`-CameraName` を渡すと 28 件）。前半7件は上記の循環待ちの回帰検証: 4K 画面キャプチャ構成（`d3d12screencapturesrc monitor-index=<n>`・`qsvh264enc rate-control=vbr bitrate=2000 max-bitrate=3000 gop-size=60`・`BufferDuration=10000`）、`d3d12testsrc` による解像度スイープ（320x240 / 1920x1080 / 2560x1440 / 3840x2160 ── 1920x1080 以下は閾値未満、2560x1440 以上が「1 フレーム ＞ 5242880 バイト」の循環待ちの領域。スクリプト自身もこの式で各行に注記を付ける）、4K での自動選択（停止した候補が受理されず棄却され、次の候補が試されること）。スイープが `d3d12screencapturesrc` ではなく `d3d12testsrc` なのは、結果をその機械のモニタ構成に依存させないため。**スイープとは別に、同じ 4K ソースを製品が式で決める値（`bitrate=12442 max-bitrate=18663`）で回す 1 件を足してある** ── スイープ側は「解像度だけが違う」ことが価値なので触らず、比較はこの 2 行のバイト数と尺で行う。既定の 2000 は kbit/sec でも bit/sec でも「ありそうな数」なので単位誤りを隠すが、12442 はそうではない。エンコーダー行はカタログと `EncoderCatalogScriptSyncTests` で結び付けてあり、カタログを変えるとテストが落ちてこの行の更新判断を迫る。
...
**引数無しで走らせると、GPU の無い機械では `capabilities` が FAILED になり、以降のケースは
SKIPPED になる** ── それがこの機械での正しい結果である（同梱ランタイムにソフトウェアの
H.264 デコーダーは無い）。

**開発機でも `-GstBin` と `-H264Decoder` を渡せば 6 ケースすべてを流せる。** ワーカーの
`PATH` の先頭へ、ソフトウェアの H.264 デコーダーを持つ別のランタイムの `bin` を置き
（ローダーは PATH の走査を最優先で解決し、勝った `bin` のランタイムを丸ごと選ぶ）、
使う要素名を名指しする:

```powershell
.\tools\Verify-Transcode.ps1 `
  -GstBin "$env:LOCALAPPDATA\Programs\gstreamer\1.0\mingw_x86_64\bin" -H264Decoder openh264dec
```

開発機での実測は 6 ケースすべて OK（`transcode-start0` は 767,694 bytes を 1.0 秒、
`transcode-seek` は 468,837 bytes）。**この走らせ方は同梱物についての言明にはならない**
── 当たっているのは外のランタイムで、ハードウェア デコーダーのメモリ交渉も踏まない。
...
- **`d3d12h264enc` / `amfh264enc` にプリセットのビットレートが効かないこと ── 現在の制約（直していない）。** この 2 種は `BitrateUnitPerKbps` が null なので `WithBitrateKbps` が素通しし、その機械では**ライブ DASH でも録画トランスコードでも効くのは GOP・解像度・fps だけ**である。プロパティ名とレート制御の nick は GPU 実機で `gst-inspect-1.0.exe <要素名>` を叩かないと確かめられないので、カタログへ足すのは実機で確認してからにする。効いていないことはログからは分からない（`dash.stream-start` の `kbps=` は設定から読んだ要求値）。Intel / NVIDIA の 4 種には `bitrate`（kbit/sec）が渡るので、この穴は**録画経路については上の `Does the bitrate reach the GPU encoder?` 節が塞ぐ** ── **DASH プレビューの `PreviewBitrateKbps` を既定以外にしたときの配信物は、GPU 機で一度も測っていない**（手動）。
