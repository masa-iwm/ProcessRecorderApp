...
- **NuGet のグローバルパッケージフォルダのキャッシュ**（`actions/cache`、`~/.nuget/packages`）を `build-and-test` と `publish-aot` の「Set up .NET」の直後に置く。キーは `nuget-<os>-<hashFiles>` で、入力は各 `*.csproj`・`Directory.Packages.props`・`Directory.Build.props`・`*.pubxml`・`nuget.config`・`global.json`（SDK の版が同梱の暗黙パッケージの版を決める）。**リポジトリのキャッシュ枠は 10GB で、超えた分は最後に使われた時刻の古い順に消される** ── 枠を食い潰すと次に要るエントリが先に消えるので、エントリを小さく・少なく保つ形にしてある: (1) **復元（`actions/cache/restore`）と保存（`actions/cache/save`）を分け、保存は `build-and-test` が main で、キーが完全一致しなかった回にだけ行う。** ブランチのエントリは main から読めないので保存しても枠を食うだけで、ブランチは main のエントリを読める。(2) **main では接頭辞フォールバック（`restore-keys`）を使わない** ── 古いエントリを土台に復元してから保存すると、使わなくなった版が残ったまま積み上がり、エントリが単調に膨らむ（`.nupkg` 込みのクリーンな 1 本で圧縮後 約 1.2GB。除去後の値は初回保存の Cache Size で差し替える）。保存しないブランチと `publish-aot` は膨らむ先が無いので、接頭辞 `nuget-<os>-` まで落として差分だけ取りに行く。(3) **保存の直前に `.nupkg` 本体を消す** ── NuGet が「展開済み」と判定するのは `.nupkg.metadata` で、本体が無くても restore / publish は何も取り直さない。本体は展開済みフォルダの約 3 割（2.4GB 中 0.7GB）で、圧縮済みなのでエントリの大きさにそのまま載る。`path` の除外パターンでは落とせない（`actions/cache` はディレクトリを丸ごと tar へ渡す）。(4) **エントリはキーごとに 1 本** ── `publish-aot` は同じキーを読むだけで保存せず、ILCompiler（展開後 約 100MB）を毎回取り直す。専用エントリを持つと、中身の 9 割以上が重複した 1 本がキーごとに増える。**`dotnet restore` / `dotnet publish` の書き方は変えない** ── 発行に `--no-restore` を使えない理由（`.pubxml` にしか無い `PublishReadyToRun` / `PublishAot`）はそのまま残るので、キャッシュは復元を**速くするだけで省かない**。したがって**古いキーに当たっても壊れない**（足りないパッケージだけ取りに行く）。`e2e` ジョブには置かない ── 下記のとおり、あちらはもう何も復元しない。
...
5. **成果物は形態とシャードで名前を分ける**（`test-results-<flavor>-<shard>` ほか）。matrix の 6 ジョブが同じ名前で上げると衝突する。スクリプトは子プロセスの標準出力を `tests/ProcessRecorderApp.E2E/TestResults/e2e-<shard>.log` へ落とすので、**赤い回はこのファイルを見る**（子の console 出力はステップのログに出ない。要約表だけがステップに出る）。`test-results-*` は**緑でも上げる**（`!cancelled()`）── DASH E2E が合否に関わらず trx へ残す余裕の軌跡（`dash smoothness trace`）が、先行バッファを決める較正データになる。`e2e-diagnostics-*` は失敗時だけ（`crash-dumps-*` は常に）。
...
- 同梱 MinGW（`ProcessRecorderApp-<tag>-win-x64-gstreamer-mingw.zip`）── 削減済み runtimes（46 ファイル・49.8MB）を同梱する。**自己完結**（libstdc++ / libgcc / libwinpthread も入る）。
- 同梱 MSVC（`ProcessRecorderApp-<tag>-win-x64-gstreamer-msvc.zip`）── 同じ選択の MSVC ビルド（44 ファイル・24.5MB）。小さく、`capture-api`（WGC）が使えるが、**利用者の機械に VC++ 再頒布可能パッケージが要る**（`msvcp140` / `vcruntime140` / `vcruntime140_1` は同梱しない）。
