...
**t+4 秒以降の `waiting` の増分が 1 回以下**・**余裕が一度は 1.0 秒以上になる**こと（主判別は前者。後者は開発機の実測 ── 旧値では 0.80 秒が上限、新値では 1.26〜2.02 秒 ── に合わせた補助で、join がバーストする機械では旧値でも通り得る）。
**停止の総数でも終了時の余裕でも見ない** ── join では必ず 2〜3 回止まる（空のリングから
始まるので構造的）し、寄せは前へしか跳べないので終了時の値はドリフトのぶんだけ振れる。
残るのは次の 5 つ:
