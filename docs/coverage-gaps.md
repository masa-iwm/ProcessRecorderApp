...
`TrayMenuTests` がウィンドウの外のメニューを実際に開いて resw の値と照合するが、`Category=Fragile` で **CI の必須ゲート（`tools/Run-E2E.ps1 -ExcludeFragile` が全シャードに同じ節 `Category!=Fragile` を足す）からは外してある**。守られるのは手元でフィルタなしで回したときだけで、「push すれば守られる」とは読まないこと。除外の理由はシェル側（オーバーフローのトグル・物理カーソルでの右クリック）であって製品ではない。
...
そこで守られている。**滑らかさ**（止まっては跳ぶ形になっていないこと）は
`WebUiBrowserTests.TheDashPreviewPlaysWithoutStallingBehindTheLiveEdge`（1 件）が
`waiting` の回数で見る ── 18 秒再生して、進み 15 秒以上・
**末尾 5 標本（1 秒ごとに 1 標本）での `waiting` の増分が 1 回以下**であること。
**停止は時刻ではなく末尾で切る**
── join の停止が出切る時刻は機械ごとに前後するので、固定の窓では位相しだいで振れる。
**18 秒に延ばしてあるのは、定常への遷移そのものが停止を伴うためである**
── 遅い機械では遷移が t+9 まで来る。短い窓だと、この遷移が末尾へ掛かって
「定常で止まっている」ことにされる。**停止の総数では見ない** ── join 直後は 1 回止まりうる
（クライアントは 2 秒溜めてから再生するが、空のリングに居合わせた視聴者はそこから育てる）。

**ライブ端の余裕（cushion）の大きさは断定していない。** セグメントは 1 秒に 1 本しか
生まれず、余裕は届くたびに跳ねて秒あたり 1 秒ずつ減る鋸歯なので、値は append との位相で
決まる ── 「どこかで最大が閾値に届くこと」は、ライブ端へ寄り切った形でも 1 本の append で
満たされてしまい、健全な定常と分けられない（実測: 瞬間値が 0.4〜0.5 秒に張り付いた 8 秒の
毎秒の最大が 1.05〜1.35 秒で、定常の 1.25〜1.48 秒と重なる）。**製品側にも余裕を保証する
機構が無い** ── 追従が動くのは遅れが 4.5 秒を超えたときだけで、join 時に先行バッファを
作るかは未決である。余裕について断定するのは 2 つだけ: **停止が 1 度も無い末尾 5 標本では最小が正であること**
（止まらずに進んだのに余裕が負なら、測っている対象が違う。停止の最中は再生位置が
`buffered.end` に並ぶので、停止を許した窓では見ない）と、**標本器が走っていること**
（1 秒に 5 個以上）。余裕はページの中に仕掛けた 50 ms の標本器で測り、
**毎秒の最大と最小を合否に関わらずテスト出力へ残す**（先行バッファを決めるときの較正データ）
── 1 秒周期で読むと、位相で決まる定数しか見えない（健全な再生が、位相しだいで
全標本 1.15 秒にも 1.33/0.30 の交互にも見える）。18 標本の軌跡も**合否に関わらず**出る。
残るのは次の 5 つ:

- **ライブ端へ「2.5 秒後ろ」に寄せる経路（遅れ 4.5 秒超で跳ぶ）は E2E では構造的に通らない。**
  E2E は毎回新しいインスタンスを立てるのでブラウザは常に最初の視聴者で、リングは空に
  近い状態から始まる。遅れが 4.5 秒を超えるのは満杯のリング（6 秒）へ後から join したときか
  タブがスロットルされたときだけで、上のテストが守るのは「旧値 0.5 秒への再寄せが**無い**」
  ことまで。寄せ先を 0.5 へ戻す退行はどの機械でも緑で通る ── 2 人目の視聴者として開く
  GPU 機の手動確認（cushion が 2 秒前後で落ち着くこと）が対になる。
- **別 PC・実 GPU のエンコーダー候補での DASH は無検査。** 手元と CI が通すのは
  `EncoderCatalog` の並びのうち、その機械で実際に採用された 1 つだけである
  （手動項目として `docs/gpu-verification.md` に 1 行ある）。滑らかさも同じで、
  **GPU エンコーダーでの実測は手動**である ── 上の E2E が測るのは開発機の
  ソフトウェア エンコーダーでの 1 経路だけ。
...
- L2 `TranscodeTests`（7 件）── 変換された本文が `ftyp`＋`moov` に続く `moof`/`mdat` で
  あること、要求した位置から始まること、高さがソースへ丸められること、ライブ DASH と
  1 つの枠を取り合うこと、同じ `session` のシークが枠を引き継ぐこと。加えて
  **プリセットの fps がソースの実 fps を上回る本**が通ること ── ソース `15/1` で **sidecar を消したもの**、
  ソース `89/3`（≈29.67、C920 ネイティブの値）で sidecar 有りの 2 通り
  ── と、**常時録画のセグメント**（枝を 160×120・5fps にし、720p を要求したもの）の sidecar が枝の実体
  （幅・高さ・fps）を持ち、そのセグメントが変換できること。さらに **GOP を交渉済みの実 fps で
  決め直す経路**（`ATranscodeOfALowFramerateSource_KeepsEveryFragmentSeekable` ── ソース `5/1`・
  sidecar 無しで 720p を要求し、`moof` が同期サンプルで始まることを断定する）。
...
- **GOP の決め直しを断定するのは 5fps の 1 本だけ** ── `TranscodeSession.Retune` の
  「組み直す」枝を実際に通すのは上の 1 件で、**フォールバック 3 通り**（preroll が
  `PrerollTimeoutMs` に間に合わない・交渉済み caps に `framerate` が無い・新しい GOP で
  エンコーダーを解決できない）はどの層でも走らない ── 通ったかどうかは
  `transcode.start` の `gop-source=` にしか出ない（`quality` / `negotiated` / `fallback`
  ── `negotiated` は「交渉済みの実 fps で決めた」であって、要求と同値なら組み直していない）。
  **組み直しの最中に同じ `session` の次の要求（＝シーク）が刺さる並び**も作れていない
  （2 本目のパイプラインは `Start()` 末尾の柵が畳む形にしてある）。
- **`fragment` の長さは GOP に依らない** ── `mp4mux fragment-mode=dash-or-mss` は
  `fragment-duration` のとおり 1 秒で切るので（実測: GOP 6 秒でも `moof` は 1 秒ごと）、
...
sidecar と一覧の `trigger` のうち、**自動で通るのは `remote` / `cli` / `continuous` の 3 つだけ**
である（`RecordingDeliveryTests` と `WebUiBrowserTests` が、HTTP・CLI・常時録画のそれぞれから
始めた録画を見る）。残り 2 つは E2E から発火できない。

- `manual` ── GUI のトグル・ボタンからの開始。L3（UIA）は録画の開始を押さないので、
  この値を書く経路（`GstEventRecorderViewModel`）は自動では 1 度も通らない
- `uia:<triggerId>` ── UIA トリガからの開始。発火そのものが自動化できない
  （上の「UIA トリガの実発火経路」と同じ理由）

手動確認: `manual` は GUI から 1 本録って `<録画ファイル名>.mp4.json` の `trigger` が
`"manual"` であること、`uia:<id>` は「UIA トリガの実発火経路」の手順 2 のあとで同じファイルを
見ること。どちらも Web UI の録画一覧の Trigger 欄にそのまま出る。

カレンダーの**夏時間の境界**も手動である。`recording-days` へ渡す `tz` はクライアントの
**現在の**オフセット 1 つなので、夏時間をまたぐ月では境界の 1 時間ぶんの録画が隣の日に
数えられ得る ── 開発機も CI も日本時間（夏時間なし）なので、E2E ではこの窓に入れない。
...
### UI スレッドへ乗るまでの期限（`RunOnUiAsync`）

`RemoteControlBackend.RunOnUiAsync` は **UI スレッドへ乗るまで**に 30 秒の期限を持ち、
超えると 503 ＋ `Retry-After: 2` ＋ 本文 `ui thread busy` を返す。**この期限が実際に切れる形は
どの層でも走らない** ── UI スレッドを 30 秒塞ぐ手立てが無く、E2E から注入もできない。
固定してあるのは L1 `RemoteApiRulesTests.TheBackendGivesUpWhenItCannotGetOntoTheUiThread`
のソーステキスト検査だけで、見ているのは「期限があること・待つ対象が『乗ったこと』であること・
降りた後の例外を観測していること」の 3 点である。

**測るのが「乗るまで」であって「終わるまで」でないことが要点である。** 停止は
`EventRecorder.MaxAdvisedStopFinalizeTimeoutMs`（50 秒）まで正当に掛かるので、全体へ期限を
掛けると正常に遅い停止を「塞がっている」と偽って断る。ここを触るときはその境目を崩さないこと。

### 自動化を見送っているもの（注入の手立てが無い／得られる保証が小さい）

- **`TranscodeReader.TryRead` のイベント駆動化。** いまは `Task.Run` 越しに呼んで、
  スレッドプールのスレッドを最大 `TranscodeReadTimeoutMs` 塞ぐ。塞いだままの本数が
