...
`waiting` の回数とライブ端の余裕で見る ── 18 秒再生して、進み 15 秒以上・
**末尾 5 標本（1 秒ごとに 1 標本）での `waiting` の増分が 1 回以下**・
**全 18 標本の余裕の最大が 1.2 秒以上**であること。**停止は時刻ではなく末尾で切る**
── join の停止が出切る時刻は機械ごとに前後するので、固定の窓では位相しだいで振れる。
**18 秒に延ばしてあるのは、定常への遷移そのものが停止を伴うためである**
── 遅い機械では遷移が t+9 まで来る（それまで余裕 0.9 秒前後で停止ゼロ、遷移で 1.9 秒台へ跳ね、
以降は動かない）。短い窓だと、この遷移が末尾へ掛かって「定常で止まっている」ことにされる。
**余裕は末尾では見ない** ── 寄せは前へしか跳べないので終わりの値はドリフトのぶんだけ
振れ（実測 0.58〜1.98）、落ち着いた再生でも下がって終わる。閾値 1.2 は実測の
2 つの帯のあいだから採ってある（届く側の下限 1.26 秒、ライブ端に張り付いた側の
上限 1.05 秒）。**停止の総数では見ない** ── join では必ず 2〜3 回止まる
（空のリングから始まるので構造的）。18 標本の軌跡は**合否に関わらず**テスト出力へ出る
（緑の run も閾値の較正データになる）。
残るのは次の 5 つ:
