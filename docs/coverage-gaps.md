...
1. **右クリックメニューの中身** ── `MenuFlyout` は**別のトップレベル UIA ウィンドウ**に出る
   ...
   レコーダー一覧が出ること・選ぶと切り替わること・「全画面を終了」で戻ることを目で見る。
   **通常表示のときに「全画面を終了」が出ないこと**も併せて見る
   （出ていれば押しても何も起きない死に項目 ── ここは自動では一切観測できない）。
2. **左右キーでのレコーダー切替** ── フォーカスの位置に依存する（アクセラレータは
   ウィンドウ全域に効くが、フォーカスが WebView2 の中にあると届かない）。
