...
- **`d3d12screencapturesrc show-cursor=true` はプロセスごと落としうる**（上流の欠陥）。カーソル形状を組み立てる `PtrInfo::BuildTexture` が `std::vector::resize` を投げっぱなしにしており、C++ 例外が抜けると `std::terminate` → `abort` になる。実測（フルダンプの例外レコード）: `0xc0000409` パラメータ `0x7`（FAST_FAIL_FATAL_APP_EXIT）、故障スレッドはネイティブ、スタックは `gst_d3d12_screen_capture_src_dxgi_capture` → `gst_d3d12_dxgi_capture_do_capture` → `DesktopDupCtx::Execute` → `DesktopDupCtx::ExecuteInternal` → `PtrInfo::BuildTexture` → libstdc++。**アプリ側では捕捉できない**（別スレッドのネイティブ `terminate`）。既定は `show-cursor=false`。
