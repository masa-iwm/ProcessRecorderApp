...
- `dwriteclockoverlay`（`gst-plugins-bad` の dwrite プラグイン、`libgstdwrite.dll`。同梱ランタイムにも含まれる）は cairo を経由せず DirectWrite を直接使う。sink caps は `video/x-raw(ANY)` なのでシステムメモリ経路でも使える。プロパティは `time-format` / `auto-resize` / `font-family` / `font-size`（`font-desc` は pango 専用なので無い）。**`auto-resize=false` は必ず渡す**（既定の TRUE では文字の大きさがフレームの大きさに追従する）。GStreamer 1.28.6 以前では `auto-resize=false` を渡すと `GLib-GObject-CRITICAL ... G_VALUE_HOLDS_UINT` が標準エラーに出る（実測: 1.28.6 の MSVC 版）が、1.28.7 では出ない（実測: 両形態・削減前後の木）。
