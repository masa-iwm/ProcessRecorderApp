...
- **モニターの物理ピクセルは `EnumDisplaySettings`（`ENUM_CURRENT_SETTINGS`）で取る。** ディスプレイの現在のモードなので、プロセスの DPI 認識に依存しない（DPI 非対応のテストホストで 3840x2160 を実測。`GetMonitorInfo` は同じ機械で 2194x1234 を返す）。**GStreamer のデバイスプロバイダで取ってはいけない** ── `gst_device_provider_get_devices` の GList を辿る経路でメモリを壊し、パイプライン編集ダイアログでソースを切り替えただけでアプリが落ちた（`Microsoft.UI.Xaml.dll` の `0xc0000005`、および `GetDynamicChoices → ToArray` でのアクセス違反を実測）。`LibraryImport` は `ByValTStr` を含む構造体を扱えない（SYSLIB1051）ので、`DISPLAY_DEVICE` / `DEVMODE` は `fixed char` で blittable にする。
