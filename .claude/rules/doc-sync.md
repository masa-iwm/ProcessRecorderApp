...
| `DashStopReasonTests` | `DashPreviewStream.StopReasons`（`dash.stream-stop` の `reason=` の正本・10 個）⇔ `StopReason` の `const` 群（リフレクションで**過不足なく一致**。定数を足して配列へ入れ忘れると落ちる）⇔ `src/README.md` の**2 箇所**（停止理由の表の `reason` 列は**双方向**: 行を消しても要素を消しても落ちる／ログイベント表の `dash.stream-stop` 行の `｜` 区切りの列挙と「N 種」の個数）／`DashPreviewStream.cs` のソーステキストに直書きされた `Teardown("…")` と `Fault(engine, "…")` の理由が `StopReasons` に含まれること（**コメント行は除外**。現状 0 件） |
...
| `E2EShardSyncTests` | E2E の `[Trait("Category", "Gui")]` ⇔ `AppUi.` / `TrayUi.` の使用（**ファイル単位・双方向**。使っているのに付いていない／使っていないのに付いている、のどちらでも落ちる。メソッドレベルのトレイトは使わない。**検査はこの 2 つの名前のテキスト出現だけ**なので、E2E からの UIA アクセスは必ず `AppUi.` / `TrayUi.` を経由すること ── 直に UIA を触ると無検査になる）／`tools/Run-E2E.ps1` の `$WebShardClasses` ⇔ E2E に実在する `public sealed class`（`[Collection(E2ECollection.Name)]` を持ち、Gui トレイトを持たず、**他のクラス名の末尾部分文字列でない**こと ── フィルタは `E2E.<Class>.` と前後を付けて書くので `E2E.RecordingTests.` は `E2E.ContinuousRecordingTests.Method` に当たらない。この末尾一致検査が守るのは**裸のクラス名を書いた場合**（`FullyQualifiedName~RecordingTests` は `ContinuousRecordingTests` まで巻き込む）である）／`.github/workflows/build.yml` の `shard:` matrix ⇔ スクリプトの `ValidateSet`（`all` を除く）と `{gui, web, core}`。**シャードのフィルタは空振りしても `dotnet test` が成功するので、崩れた形は「速くなった」ようにしか見えない** |
