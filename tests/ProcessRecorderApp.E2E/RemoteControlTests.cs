...
/// </para>
/// <para>
/// <b>画面（UIA）を触るケースは <see cref="RemoteControlUiTests"/> に分けてある</b>
/// ── <c>AppUi</c> を使うファイルはクラスに <c>Category=Gui</c> を持ち、CI では
/// 実デスクトップの runner にまとめて流す（L4 の <c>E2EShardSyncTests</c> が突き合わせる）。
/// <b>このファイルに <c>AppUi</c> / <c>TrayUi</c> を書き足してはいけない。</b>
/// 補助はあちらから使えるように <c>internal static</c> で持っている。
/// </para>
/// </summary>
...
public sealed class RemoteControlTests(PublishedApp app, ITestOutputHelper output)
{
    ...
    /// </summary>
    internal const string Token = "E2E-remote-control-token-0123456789-abcdefg";
    ...
    internal static readonly TimeSpan StartBudget = TimeSpan.FromSeconds(30);
    ...
    internal static readonly Regex BindPattern = new(@"\bbind=([0-9.]+):(\d+)\b", RegexOptions.Compiled);
    ...
    internal static SettingsFile RemoteSettings()
    {
        var settings = RemoteBase();
        settings.AddRecorder("R1");
        settings.AddRecorder("R2");
    ...
    /// </summary>
    private int WaitForPort(AppInstance instance) => WaitForPort(instance, output);

    /// <inheritdoc cref="WaitForPort(AppInstance)"/>
    /// <remarks>
    /// <c>RemoteControlUiTests</c>（別クラス）が同じ待ちを使うので静的で持つ。
    /// </remarks>
    internal static int WaitForPort(AppInstance instance, ITestOutputHelper output)
    {
    ...
    /// </summary>
    internal static HttpClient CreateClient(int port) =>
    ...
    internal static HttpRequestMessage Post(string path) => new(HttpMethod.Post, path);
    ...
    internal static async Task<string> ReadEventAsync(StreamReader reader, string eventName, TimeSpan budget)
    {
        var deadline = Stopwatch.StartNew();
        string? current = null;
    ...
    internal static readonly TimeSpan DebounceMargin = TimeSpan.FromSeconds(3);
    ...
    private Task<JsonDocument> ExpectAsync(HttpResponseMessage response, HttpStatusCode expected)
        => ExpectAsync(response, expected, output);

    /// <inheritdoc cref="ExpectAsync(HttpResponseMessage, HttpStatusCode)"/>
    /// <remarks>
    /// <c>RemoteControlUiTests</c>（別クラス）が同じ読み方を使うので静的で持つ。
    /// </remarks>
    internal static async Task<JsonDocument> ExpectAsync(
        HttpResponseMessage response, HttpStatusCode expected, ITestOutputHelper output)
    {
        string text = await response.Content.ReadAsStringAsync(Ct);
