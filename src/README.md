...
配布形態が同梱・非同梱の2種類あるため、同梱物の決め打ちはできない。次の優先順で探す。

1. 元の `PATH` に既に入っているディレクトリ
2. `%GSTREAMER_1_0_ROOT_MINGW_X86_64%\bin`
3. レジストリのアンインストール情報から見つけた GStreamer(MinGW) の `bin`
4. MSYS2 の `ucrt64\bin`（レジストリ →`%SystemDrive%\msys64` →`%LOCALAPPDATA%\Programs\msys64`）
...
全候補に `gop-size=15`（`x264enc` は `key-int-max=15`）を明示している。
...
| **`gop-size=15` へ変更後** | **1.0秒** | **4.3〜6.1秒**（＝録画窓＋事前バッファ） |

フレームレートは利用者の `SrcPipeline` 側にあり `EncoderCatalog` からは分からないため、
GOP は固定値とし、想定される最低フレームレート（15fps）でも GOP 間隔が 1 秒に収まる
`15` を選んでいる。`EncoderCatalogResolveTests.EveryCandidate_PinsAShortGopLength` が
この不変条件を守る。
