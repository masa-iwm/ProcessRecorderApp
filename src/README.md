...
| 録画・プレビューエンジン | **GstSharp.Net**（`GstSharp.Net` / `GstSharp.Net.App` / `GstSharp.Net.Base` / `GstSharp.Net.Video` 1.28.11、GStreamer の .NET バインディング。nuget.org から取得する。後述「パッケージの取得元」） |
...
| モニタが 1 台も列挙できない ＋ `monitor-index` が書かれている | パスだけ取り除いて `monitor-index` を残し、**警告**（`recorder.warning` と `LastError` の `[warning]` 接頭辞） |
| モニタが 1 台も列挙できない ＋ `monitor-index` が**無い** | **初期化を失敗させる**（理由は「繋がっていない」とは別の文言） |
| 一致したがハンドルを読めない | 同上（`monitor-index` の有無で縮退／失敗が決まる） |
...
> **逆に、列挙が空でも `monitor-index` が書かれていれば失敗させてはいけない** ──
> 利用者の GStreamer に d3d12 が無いだけの機械で、番号なら録れるのに 1 フレームも
> 録れなくなる。縮退したことは画面からは分からないので、そのときは黙らせずに警告として残す。

> **縮退できるのは `monitor-index` が書かれているときだけ。** パスだけで指定するのが本来の
> 使い方なので、番号が書かれていないことはむしろ普通である。そこでパスを取り除くと
> `d3d12screencapturesrc capture-api=wgc ! …` のように**選択プロパティが 1 つも無い**文字列に
> なり、要素は**既定のモニタ（index 0）を黙って撮る** ── `monitor-device-path` が防ごうと
> している事故そのものを、縮退の側で作ることになる。**戻せる先が無いなら縮退せず失敗させる。**
> 失敗すれば初期化が失敗し、デバイス到着の監視（`TryScheduleDeviceRebuild`）が拾い直して
> 復帰するので放置にはならない。
>
> この失敗の理由は「繋がっていない」（一致しない場合）とは**別の文言**にしてある ──
> 原因が違う（あちらはモニタが無い、こちらは列挙できない／ハンドルが読めない）ので、
> 同じ文言だと利用者が「挿し直す」という無効な対処へ誘導される。文言には
> 「番号でよければ `monitor-index` を書けば縮退できる」ことも入れてある。
...
- **読む位置は必ず buffer の中に収める。** 平面ごとに、最終行は行バイト数まで
  （`offset + stride × (行数 − 1) + 行バイト数`）が map した長さに収まらないものは撮らない
  （`thumbnail.unsupported reason=buffer-too-short`。必要長は `required=` に出る）
- **扱えないレイアウトは撮らない。** 1 行の画素に足りない stride・負の stride・
  負の offset・平面の数が足りないものは、長さでは説明できない失敗として分けて記録する
...
| `splitter fault` | `Fmp4SegmentSplitter` が壊れた入力を見つけた（内訳は `dash.stream-error` の `detail=`） |
| `stream error` | 押し込みか mux スレッドで例外が出た（内訳は `dash.stream-error` の `detail=`） |
| `close` | レコーダーの停止（`CloseCore`。枝を静止させた後） |

**`reason=` はこの 10 個で全部である**（`DashPreviewStream.StopReasons`）。切り出し器・集約器の
自由文と例外の `Message` は `reason` に入れず、`dash.stream-error` の `detail=` へ出す ──
そうしないと「起きうる理由」を運用側が数え上げられなくなる。**`parse_launch` の失敗と
`PLAYING` になれなかった場合は、この表に現れない** ── まだ mux が無いので停止する対象が無く、
記録は `dash.stream-error` の 1 行だけになる（次のサンプルで次の候補が試される）。
...
**HTTP へは `DashEndpoints` が単一ルート `GET /api/recorders/{id}/dash/{file}` で配る。**
`{file}` の解釈は `Components.DashRoutes.TryParse` の純関数 1 つで、`manifest.mpd` /
`init.mp4` / `seg-<10進数>.m4s` の 3 つだけを受ける（**名前の正本は `DashManifest` の
`InitializationTemplate` / `MediaTemplate`** ── `"seg-"` と `".m4s"` を 2 か所に持たない）。
それ以外は `MapFallback` と同じ形の 404（終了コード 4）で、`DashRoutesTests`（L1）が
受理と拒否の両側を縛る。

- **まだ始まっていない**（init も 1 セグメントも揃っていない）→ **503 ＋ `Retry-After: 5`**、
...
**速度メニューは録画の再生だけに出る**（`caps.speed` ── ライブプレビューでは `false` で
...
**`Period@id`（generation）が変わったら完全に作り直す** ── 前の init では新しいセグメントを
...
**再生の開始は `buffered` が 2 秒（`DASH_JOIN_LEAD_SECONDS`）溜まってから**で、それまで
`<video>` の autoplay を切って止めておく（`stopPreview` が戻す。chunked 側は autoplay に頼る）。
最初の視聴者が居合わせるリングはほぼ空で、1 本目で走り出すと余裕 1 秒からセグメント長 1 秒 +
ポーリング位相 0.25 秒ぶん減って谷が 0.1 秒（15 fps の約 2 フレーム = 復号器の underflow）に
なり毎秒止まる。2 秒なら谷は 0.6 秒前後（L1 が 2 本ぶん以上であることを見ている）。
状態表示は `playing` を受けるまでが `DASH: buffering (n)`、以後が `DASH: live (...)`
（E2E はこの接頭辞を再生開始の印にする。`readyState` は溜めている最中に 2 へ達する）。
...
本文が末尾まで届いた時点で `endOfStream()`。

追従（ライブ端への寄せ）は**最初の 1 回と、遅れが開いたときだけ**である ── 再生できる最初の
時点で `buffered.end` − 1 秒へ 1 度寄せ、以後は通常再生に任せて、遅れが 3 秒を超えたときだけ
`buffered.end` − 1.5 秒へ寄せ直す。**`updateend` ごとに寄せてはいけない** ── フラグメントは
1 秒に 1 つ来るので、そのたびにシークするとデコーダーが戻されて再生がカタつく。寄せる前に
`readyState` を見るのも同じ理由で、まだ 1 枚も出ていない時点でシークを積むと再生の開始が遅れる。
**利用者がシークしたら追従をやめる**（自分で寄せた位置は覚えてあり、それだけは利用者の操作と
数えない ── 位置の一致は 0.5 秒の幅で見る。`seeking` が届く時点では、要求した位置を
...
画面側の挙動のうち**ログインへの切り替え・ゲストの取り消し・追いかけ再生の進行**は
L2 の `WebUiBrowserTests` がヘッドレスの Edge で実際に動かして見る（残りは
[docs/coverage-gaps.md](../docs/coverage-gaps.md)）。
...
- **梯子の仕切り直しは作り直しだけの連鎖に絞らない。** エスカレーションの作り直しも
  到着で早められるので、そこで空振りしたときこそ次を早く来させたい ── RDP の
  セッション復帰は**仮想ディスプレイが先に現れ、実モニタは後から戻る**二段構えなので、
