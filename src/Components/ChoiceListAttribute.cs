...
/// </para>
/// <para>
/// <b>列挙型のプロパティに付けてもよい</b>（<c>PropertyGridView</c> はこの属性を
/// 列挙型の判定より先に見る）。用途は「保存値は列挙型の名前のまま、表示だけ訳す」こと
/// ── 保存値が名前であることは変わらないので設定 JSON の形も
/// <c>docs/settings.schema.json</c> も動かない。<b>プロバイダーが空を返したときは
/// 自由入力ではなく列挙型の一覧へ倒れる</b>（打ち間違いで値を落とす道を作らないため）。
/// </para>
/// <para>
