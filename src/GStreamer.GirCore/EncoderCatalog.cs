...
public static class EncoderCatalog
{
    ...
    ///
    /// <para>
    /// <b>優先順位は <see cref="Nvidia"/> の後ろに置く。</b> 明示的な2つは実機で
    /// 有効な MP4 を出すことを確認済みだが、
    /// <b>この要素はまだ一度も実機で走っていない。</b> 未確認のものを確認済みのものより
    /// 前に置くと、<b>NVIDIA 機で選ばれるエンコーダーが根拠なく変わる。</b>
    /// ここに置けば「明示の2つが両方だめだったときの最後の NVIDIA 経路」として増えるだけで、
    /// 既に動いている機械の挙動は変わらない。
    /// </para>
    /// <para>
