...
restore には `GITHUB_PACKAGES_USER` / `GITHUB_PACKAGES_TOKEN`（`read:packages` スコープ）が要る
── `UiaTrigger.*` を GitHub Packages から取得するため。専用 PAT は作らず gh のトークンに
スコープを足す（src/README.md「UiaTrigger パッケージの取得」）。
