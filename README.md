...
- **Watch a recording while it is still being recorded** — turn on `FragmentedOutput` and
  every file the app writes — event recordings and continuous segments alike — stays readable
  from the first byte, both while recording and after a forced shutdown; the browser page can
  then play it and keep following it as it grows.
